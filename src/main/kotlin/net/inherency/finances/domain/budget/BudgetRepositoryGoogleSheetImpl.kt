...
@Repository
class BudgetRepositoryGoogleSheetImpl(private val googleSheetClient: GoogleSheetClient): BudgetRepository {

    private val log = LoggerFactory.getLogger(BudgetRepositoryGoogleSheetImpl::class.java)

    override fun readAll(): List<BudgetData> {
        return transformAllDataRowsToBudgetData(readAllRows())
    }

    override fun writeNewBudgetEntries(entries: List<BudgetData>) {
        googleSheetClient.writeAllValuesToTab(TabName.BUDGETS, entriesToListOfListOfStrings(entries))
    }

    override fun updateBudgetEntryForYearAndMonth(cmd: FixBudgetToMatchDepositsCmd, newAmount: Int): BudgetData {
